    ...
    for suite_id in suites:
        ...
        for task_id in tasks:
            for l, seed in enumerate(seeds):
                ...
                data = pd.read_csv(f"/Users/floris/Desktop/ETH/ETH_FS25/Semesterarbeit/CSHS/Deleted/seed_{seed}/{suite_id}_{task_id}.csv")
    ...
    for randomness in range(len(RANDOMNESS)):
        for i in range(len(NAME_TUNING)):
                ...
                for method_ind in range(len(METHODS_JOINT)):
                    print(norm_scores[i].shape,method_ind)
                    ax.plot(
                        np.arange(NUM_ITERS),
