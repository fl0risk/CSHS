...
from utils_modelling import read_and_preprocess_dataset_onlyOneTask, split_dataset, save_gpboost_summary
...
def assess_gp_boosting(classification: bool = False) -> tuple:
    ...
    def objective_opt(trial):
        ...
        # Train the model
        bst = gpb.train(
            params=params, train_set=train_set, num_boost_round=1000,
    ...
    plt.savefig(os.path.join(setting_path, f'predicted_vs_actual.png'))
...
def plot_feature_importance(booster: gpb.Booster, X: pd.DataFrame, path: Optional[str]) -> None:
    ...
    for importance_type in ['gain', 'split']:
        ...
        plt.savefig(os.path.join(path, f'feature_importance_{importance_type}_gpboost.png'), bbox_inches='tight', dpi=300)
...
def plot_partial_dependence(booster: gpb.Booster, X: pd.DataFrame, path: Optional[str]) -> None:
    ...
    for feature in X.columns:
        ...
        plt.savefig(os.path.join(folder, f'pdp_{feature}.png'), bbox_inches='tight', dpi=300)
...
def plot_interactions(booster: gpb.Booster, X: pd.DataFrame, path: Optional[str]) -> None:
    ...
    for i, (feature1, feature2) in enumerate(feature_pairs):
        ...
        plt.savefig(os.path.join(folder, f'pdp_{feature1}_{feature2}.png'), bbox_inches='tight', dpi=300)
...
def plot_shap_values(booster: gpb.Booster, X: pd.DataFrame, path: Optional[str]) -> None:
    ...
    plt.savefig(os.path.join(folder, 'shap_summary_plot.png'), bbox_inches='tight', dpi=300)

    for feature in X.columns:
        shap.dependence_plot(feature, shap_values, X)
        plt.tight_layout()

        plt.savefig(os.path.join(folder, f"shap_dependence_plot_{feature}.png"), bbox_inches='tight', dpi=300)
    ...
    # plt.savefig(os.path.join(folder, 'shap_interaction_plot_gpboost.png'), bbox_inches='tight', dpi=300)
    ...
    #     plt.savefig(os.path.join(folder, f"shap_interaction_plot_{feature1}_{feature2}_gpboost.png"), bbox_inches='tight', dpi=300)


def main():
    for classification in [False, True]:
        booster, X, path = assess_gp_boosting(classification)
